...
def get_now_jiage(code):
        ...
        # gp = qs.real(code)#[['name','price','pre_close','date','time']]
        gp = qs.stocks(code)
        df_a = pd.DataFrame(gp)
        df_b = df_a.stack()
        df = df_b.unstack(0)[['name', 'now', 'close', 'date', 'time']]
        return df
...
def pd_ztjytime():  # 判断是否是交易时间
        ...
        if delta1 > 0 and delta2 > 0:  # 在暂停交易的时间内
            return False  # 在暂停的交易时间范围内，返回 True
        else:
...
def do_programe(code):
        if pd_ztjytime() == False:  # 判断是否在暂停交易的时间范围内
            ...
            print(abs(now_zdie))
            if (abs(now_zdie) >= 3 and abs(now_zdie) < 3.09) or (abs(now_zdie) >= 6 and abs(now_zdie) < 6.05) or (
                    abs(now_zdie) >= 9 and abs(now_zdie) < 9.1):  # 判断现在的涨跌幅是否在目标范围内
                print('444')
                email_comment = []
                email_comment.append('<html>')
                email_comment.append('<b><p><h3><font size="2" color="black">您好：</font></h4></p></b>')
                email_comment.append('<p><font size="2" color="#000000">根据设置参数，现将监控到' + name + '(' + str(
...
if __name__ == '__main__':
    #send_Email('541085504@qq.com', '4444')
    do_programe('301017')
